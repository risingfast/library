 ...
 *      05-Oct-2002 coalesce a null author rating with '' to return authors without ratings
 *  Enhancements:
*/
...
// test if Null or All or non-Null values should be shown ------------------------------------------------------------
    ...
    if (getenv("QUERY_STRING") == NULL) {
        ...
        sprintf(caSQL, "SELECT T.ID, T.`Author Name`, T.`Author Rating` FROM (SELECT BA.`Author ID` as `ID` "
                       ...
                       "WHERE CONCAT(COALESCE(T.`Author Rating`, ''), T.`Author Name`) like '%s'; ", sFilter)
                       ;
        ...
        sprintf(caSQL, "SELECT BT.`Title ID` as 'ID' "
                       ...
                       "WHERE CONCAT(BT.`Start`, BT.`Title Name`, BA.`Author Name`) LIKE '%s' "
