/*  bookVldtCharId.c - CGI to validate and retrieve a character using a char id for the bookInquiry.html webpage
...
// check for a NULL query string ---------------------------------------------------------------------------------------
    ...
    if(sParam == NULL) {
        printf("Query string is NULL. Expecting QUERY_STRING=\"CharID=<999>\". Terminating bookVldtCharId.cgi");
        printf("\n\n");
        return EXIT_FAILURE;
    }
...
// test for an empty (non-NULL) query string ---------------------------------------------------------------------------
    ...
    if (sParam[0] == '\0') {
        printf("Query string is empty. Expecting QUERY_STRING=\"CharID=<999>\". Terminating bookVldtCharId.cgi");
        printf("\n\n");
        return EXIT_FAILURE;
    }
...
//  get the content from QUERY_STRING and tokenize based on '&' character-----------------------------------------------
    ...
    if (iCharID == 0) {
        printf("Character ID is 0. Expecting QUERY_STRING=\"CharID=<999>\". Terminating bookVldtCharId.cgi");
        printf("\n\n");
        return EXIT_FAILURE;
    }
...
// Initialize a connection and connect to the database -----------------------------------------------------------------
    ...
    asprintf(&strSQL, "SELECT BC.`Character Name` "
