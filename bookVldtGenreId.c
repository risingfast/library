/*  bookVldtGenreId.c - CGI to validate and retrieve a genre name using a genre id for the bookInquiry.html webpage
 ...
 *      17-Apr-2023 change sgServer from 102.168.0.13 to 192.168.1.64
 *  Enhancements:
*/
...
// check for a NULL query string ---------------------------------------------------------------------------------------
    ...
    if(sParam == NULL) {
        printf("Query string is NULL. Expecting QUERY_STRING=\"genreID=<99>\". Terminating bookVldtGenreId.cgi");
        printf("\n\n");
        return EXIT_FAILURE;
    }
...
// check for an empty (non-NULL) query string --------------------------------------------------------------------------
    ...
    if (sParam[0] == '\0') {
        printf("Query string is empty. Expecting QUERY_STRING=\"genreID=<99>\". Terminating bookVldtGenreId.cgi");
        printf("\n\n");
        return EXIT_FAILURE;
    }
...
//  get the content from QUERY_STRING and tokenize based on '&' character-----------------------------------------------
    ...
    if (iGenreID == 0) {
        printf("GenreID is 0. Expecting QUERY_STRING=\"genreID=<99>\". Terminating bookVldtGenreId.cgi");
        printf("\n\n");
        return EXIT_FAILURE;
    }
...
// Initialize a connection and connect to the database -----------------------------------------------------------------
    ...
    asprintf(&strSQL, "SELECT BG.`Genre Name` "
                      "FROM risingfast.`Book Genres` BG "
                      "WHERE BG.`Genre ID` = '%d';", iGenreID);
